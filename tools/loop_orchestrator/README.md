...
The sequence is fixed: every checkpoint verifier, then a distinct Phase-wide verifier over the cumulative delta, and only after both pass does the engine invoke the manifest's named profile from the existing deterministic `project_harness`. Its fixed Python argv arrays, `shell=False`, UTF-8 environment, retained isolated temporary directories, and bounded diagnostics remain authoritative. A passing profile yields `READY_TO_COMMIT`; ordinary failure yields `FAILED`; start failure or timeout yields `BLOCKED`.

The final harness has a durable, at-most-once start: a `PENDING_CALL` intent is persisted before the harness subprocess is ever invoked, so a crash mid-call is unambiguous on resume — it fails closed `BLOCKED` and never reruns the harness. Once the call returns, a `COMPLETE` record (exit code, report, guard, and the exact repository/protected/HEAD/staged snapshot the harness ran against) is persisted before the terminal state transition; a crash after that point still resumes deterministically into `READY_TO_COMMIT`, `FAILED`, or `BLOCKED` from the saved record alone, again without rerunning the harness. A `COMPLETE` record whose snapshot no longer matches the current worktree, or a malformed/ambiguous (including pre-upgrade legacy) harness record, blocks rather than guessing. This is single-node durability via local file persistence, not distributed exactly-once execution, and there is no automatic harness retry.
