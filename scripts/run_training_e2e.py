...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Training:")
    history = run_training(model, train_loader, val_loader, TRAINING_CONFIG, device="cpu")
    ...
    with torch.inference_mode():
        trained_output = model(example_input)
    ...
    reload_ok = torch.allclose(trained_output, reloaded_output)
    print("  PASS" if reload_ok else "  FAIL: reloaded output differs from trained model output")
    if not reload_ok:
        print("\nPHASE 4A E2E: FAIL")
        return 1
    ...
    print(f"\nPHASE 4A E2E: {'PASS' if overall_ok else 'FAIL'}")
